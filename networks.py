...
class transcript_encoder(nn.Module):
    def __init__(self):
        ...
        self.conv1d_banks = conv1d_banks(K=hp.encoder_num_banks, position='tr_encoder')

        self.max_pool = nn.MaxPool1d(kernel_size=2, stride=1)          # pytorch
        self.conv1d_1 = conv1d(hp.embed_size*hp.encoder_num_banks//2, filters=hp.embed_size//2, size=3)
        self.bn_1 = bn(hp.embed_size//2, activation_fn="ReLU")

        self.conv1d_2 = conv1d(hp.embed_size//2,filters=hp.embed_size//2, size=3) 
        #self.relu = nn.ReLU()
        #self.relu_1 = nn.ReLU()
        #self.relu_2 = nn.ReLU()
    ...
    def forward(self, inputs):
        ...
        # Max pooling
        enc = torch.transpose(enc, 1,2)
        enc_zero = torch.zeros(hp.batch_size, enc.shape[1], 1).type(torch.cuda.FloatTensor)
        enc = torch.cat((enc, enc_zero), 2).type(torch.cuda.FloatTensor)
        enc = self.max_pool(enc)
        enc = torch.transpose(enc, 2,1)
...
class reference_encoder(nn.Module):
    ...
    def __init__(self):
        ...
        #self.relu_1 = nn.ReLU()
        self.bn_1 = bn(32, activation_fn="ReLU")
        ...
        #self.relu_5 = nn.ReLU()
        self.bn_5 = bn(128,activation_fn="ReLU")
        ...
        self.gru = gru(channel = 256, num_units=128, bidirection=False)
    ...
    def forward(self, inputs):
        ...
        input_t = torch.unsqueeze(inputs, 1) # transpose to (N, C, H, W)
...
class decoder1(nn.Module):
    ...
    def __init__(self,is_training = True): #여기서 shape는 Go의 shape여야 함
        ...
        self.memory_layer = nn.Linear(128 + hp.embed_size, 256, bias=False)
        self.proj_to_mel = nn.Linear(256, hp.n_mels*hp.r)

    def forward(self, inputs, memory):
        N = memory.size(0)
        processed_memory = self.memory_layer(memory)
        memory_lengths = [memory.shape[1] for i in range(N)]
        ...
        while True:
            ...
            attention_rnn_hidden, current_attention, alignment = self.attention_rnn(
                ...
                memory, processed_memory=processed_memory, mask=mask, t=t)
        ...
        outputs = outputs[:, :, -hp.n_mels:].contiguous().view(hp.batch_size, -1, hp.n_mels*hp.r)
...
class decoder2(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1d_banks = conv1d_banks(K=hp.decoder_num_banks, position='decoder2') # (N, Ty, E*K/2)
        self.max_pool1d = nn.MaxPool1d(kernel_size=2, stride=1)          # pytorch

        self.conv1d_1 = conv1d((hp.embed_size//2)*hp.decoder_num_banks, filters=hp.embed_size // 2, size=3) # (N, Ty, E/2)
        self.bn_1 = bn(hp.embed_size//2, activation_fn="ReLU")

        self.conv1d_2 = conv1d(hp.embed_size//2, filters=hp.n_mels, size=3)
        self.bn_2 = bn(hp.n_mels)

        self.dense1 = nn.Linear(hp.n_mels,hp.embed_size//2)
        self.dense2 = nn.Linear(hp.embed_size, hp.n_fft//2+1)
        self.highwaynet = highwaynet(hp.embed_size//2, num_units=hp.embed_size//2)
    ...
    def forward(self, inputs):
        ...
        # conv1d bank
        dec=self.conv1d_banks(inputs)

        # Max pooling
        dec_zero = torch.zeros(hp.batch_size, dec.shape[1], 1).type(torch.cuda.FloatTensor)
        dec = torch.cat((dec, dec_zero), 2).type(torch.cuda.FloatTensor)
        ...
        """
        # FC
        dec = torch.transpose(dec,1,2)  # N C L  -->  N, C*L (C*n_mels)
        dec = dec.view(hp.batch_size, -1)
        dec = self.dense1(dec)
        dec = torch.trnaspose(dec, 1,2)  #
        """
...
class Tacotron(nn.Module):
    ...
    def __init__(self):
        ...
        self.txtembed = embed(len(hp.vocab), hp.embed_size)
        self.transcript_encoder = transcript_encoder()
        self.reference_encoder = reference_encoder()
        self.decoder1 = decoder1()
        self.decoder2 = decoder2()

    def forward(self, text_input, mel_input):
        txtembedding = self.txtembed(text_input)       # (N, Tx) -->  (N, Tx, E)
